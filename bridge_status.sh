...
peer_id=$($binary p2p info --node.store "$celestia_bridge_path" | jq -r '.result.id')
if [ -z "$peer_id" ]; then
    send_red_alert "$server_ip"
    exit 1
fi

local_header=$($binary header local-head --node.store "$celestia_bridge_path" | jq -r '.result.header.height')
if [ -z "$local_header" ]; then
    send_red_alert "$server_ip"
    exit 1
fi

network_header=$($binary header network-head --node.store "$celestia_bridge_path" | jq -r '.result.header.height')
if [ -z "$network_header" ]; then
    send_red_alert "$server_ip"
    exit 1
fi

chai_id=$($binary header local-head --node.store "$celestia_bridge_path" | jq -r '.result.header.chain_id')
if [ -z "$chai_id" ]; then
    send_red_alert "$server_ip"
    exit 1
fi
...
semantic_version=$(echo "$version_output" | grep 'Semantic version' | awk '{print $3}')
...
output+="<b>Version:</b> $semantic_version%0A"
